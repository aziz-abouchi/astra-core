...
fn buildFromPattern(gpa: std.mem.Allocator, eg: *EGraph, pat: Pattern, env: *Env) !u32 {
    return switch (pat) {
        .Var => |v| env.get(v) orelse return error.UnboundVar,
        .App => |a| blk: {
            const sym = try symFromStr(a.sym);
            var child_ids = try gpa.alloc(u32, a.args.len);
            var i: usize = 0;
            while (i < a.args.len) : (i += 1) child_ids[i] = try buildFromPattern(gpa, eg, a.args[i], env);
            const id = try eg.addENode(.{ .sym = sym, .children = child_ids });
            gpa.free(child_ids);
            break :blk id;
        },
    };
...
pub fn applyRule(gpa: std.mem.Allocator, eg: *EGraph, rule: Rule) !bool {
    ...
    for (matches) |*env| {
        var union_target: ?u32 = null;
        var it = env.map.iterator();
        if (it.next()) |kv| union_target = kv.value_ptr.*;
        if (union_target == null) continue;
