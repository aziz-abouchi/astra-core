...
pub const RunConfig = struct {
    ...
    max_saturation_passes: usize = 64,
    enable_labels: bool = true,
};

const AirNode = struct {
    id: u32,
    op: []const u8,
...
fn costProfile(profile: RunProfile) Cost.Profile { return switch (profile) { .default => .default, .local => .local, .distrib => .distrib }; }

pub fn buildEGraphFromAirJson(
    ...
    var id_to_class = std.AutoHashMap(u32, u32).init(gpa);
    defer id_to_class.deinit();

    for (parsed.value.nodes) |an| {
        const sym = try symFromOp(an.op);
        var child_ids = std.ArrayList(u32).init(gpa);
        defer child_ids.deinit();
        if (an.children) |chs| {
            for (chs) |cid| {
                const p = id_to_class.get(cid) orelse return error.ChildNotLowered;
                try child_ids.append(p.*);
            }
        }
        var n = Node{ .sym = sym, .children = try child_ids.toOwnedSlice() };
        defer gpa.free(n.children);
    ...
    const p = id_to_class.get(parsed.value.root) orelse return error.InvalidRoot;
    return p.*;
}
...
pub fn saturate(
    ...
    while (changed_any and pass_count < cfg.max_saturation_passes) : (pass_count += 1) {
        changed_any = false;
        for (rules) |r| {
            const changed = try Apply.applyRule(gpa, eg, r);
            if (changed) changed_any = true;
        }
        _ = cfg; // pas de rebuild complet dans cette variante
    }
}

pub fn extractOptimizedSExpr(
    gpa: std.mem.Allocator,
    eg: *EGraph,
    root_eclass: u32,
    profile: RunProfile,
) ![]u8 {
    var extractor = try Extractor.init(gpa, eg, costProfile(profile));
    defer extractor.deinit();
    return try extractor.extractAsSExprString(gpa, root_eclass);
}

pub fn runFromAirJson(
    gpa: std.mem.Allocator,
    air_json: []const u8,
    rules_yaml_path: []const u8,
    cfg: RunConfig,
) ![]u8 {
    var eg = try EGraph.init(gpa);
    defer eg.deinit();
    ...
    const rules = try loadRulesFromYaml(gpa, rules_yaml_path);
    defer { for (rules) |*r| r.deinit(gpa); gpa.free(rules); }

    try saturate(gpa, &eg, rules, cfg);
    return try extractOptimizedSExpr(gpa, &eg, root, cfg.profile);
}

// ---- Test unitaire corrigé (multilignes JSON raw) ----
