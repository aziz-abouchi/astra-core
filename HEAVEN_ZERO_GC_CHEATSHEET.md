# 🧠 Heaven-Core : Cheat-Sheet Technique Zero-GC

Ce document définit les règles de gestion mémoire d'Heaven basées sur la **Quantitative Type Theory (QTT)** et les **Capabilities**. Il remplace le besoin d'un Garbage Collector par une analyse statique stricte.
...
Heaven ne scanne pas la mémoire à l'exécution. Il planifie la libération à la compilation :
...
## 4. Principes du Bootstrap (Zig ↔ Heaven)
...
* **Niveau Heaven (Logique) :** Définit les règles de haut niveau. 
* **Objectif :** Utiliser Heaven pour écrire le Typechecker d'Heaven. Cela prouve que le système de gestion mémoire est capable de gérer des structures de données complexes (arbres, graphes) sans fuite.
